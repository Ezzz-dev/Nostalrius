...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(COMBAT_PARAM_DECREASEDAMAGE)
	...
	registerEnum(ITEM_ATTRIBUTE_MOVEMENTID)
	...
	registerEnum(ITEM_ATTRIBUTE_CHESTQUESTNUMBER)
	...
	registerEnum(PlayerFlag_SpecialMoveUse)
	...
	registerMethod("Position", "sendMonsterSay", LuaScriptInterface::luaPositionSendMonsterSay);
	...
	registerMethod("Item", "getMovementId", LuaScriptInterface::luaItemGetMovementId);
	registerMethod("Item", "setMovementId", LuaScriptInterface::luaItemSetMovementId);
	...
	registerMethod("ItemType", "isChest", LuaScriptInterface::luaItemTypeIsChest);
	...
	registerMethod("ItemType", "isSplash", LuaScriptInterface::luaItemTypeIsSplash);
	registerMethod("ItemType", "isKey", LuaScriptInterface::luaItemTypeIsKey);
	registerMethod("ItemType", "isDisguised", LuaScriptInterface::luaItemTypeIsDisguised);
	registerMethod("ItemType", "isDestroyable", LuaScriptInterface::luaItemTypeIsDestroyable);
	...
	registerMethod("ItemType", "getDisguiseId", LuaScriptInterface::luaItemTypeGetDisguiseId);
	...
	registerMethod("ItemType", "getDestroyTarget", LuaScriptInterface::luaItemTypeGetDestroyTarget);
	...
	registerMethod("Condition", "setTiming", LuaScriptInterface::luaConditionSetTiming);
				...
				for (const auto& entry : indexes) {
					switch (entry.second) {
						...
						case LuaData_Teleport: {
							...
							lua_getfield(globalState, -1, "getMovementId");
							break;
						}
						case LuaData_Player:
						case LuaData_Monster:
...
int LuaScriptInterface::luaPositionSendMonsterSay(lua_State * L)
{
	// position:sendMonsterSay(text)
	const std::string& text = getString(L, 2);
	const Position& position = getPosition(L, 1);
	g_game.addMonsterSayText(position, text);
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaItemGetMovementId(lua_State* L)
{
	// item:getMovementId()
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		lua_pushnumber(L, item->getMovementId());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemSetMovementId(lua_State* L)
{
	// item:setMovementId(movementId)
	uint16_t movementId = getNumber<uint16_t>(L, 2);
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		item->setMovementID(movementId);
		pushBoolean(L, true);
...
int LuaScriptInterface::luaCreatureAddHealth(lua_State* L)
{
	...
	if (damage.value >= 0) {
		...
		damage.type = COMBAT_UNDEFINEDDAMAGE;
	}
...
int LuaScriptInterface::luaCreatureRemoveCondition(lua_State* L)
{
	...
	Condition* condition = creature->getCondition(conditionType, conditionId, subId);
	if (condition) {
		bool force = getBoolean(L, 5, true);
...
int LuaScriptInterface::luaPlayerGetDepotChest(lua_State* L)
{
	...
	DepotLocker* depotLocker = player->getDepotLocker(depotId, autoCreate);
	if (depotLocker) {
		if (!depotLocker->getParent() && player->getTile()) {
			depotLocker->setParent(player->getTile());
		}
...
int LuaScriptInterface::luaPlayerGetTotalDamage(lua_State * L)
{
	// player:getTotalDamage(attackSkill, attackValue, fightMode)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		uint32_t attackSkill = getNumber<uint32_t>(L, 2);
		uint32_t attackValue = getNumber<uint32_t>(L, 3);
...
{
	// monster:searchTarget([searchType = TARGETSEARCH_ANY])
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		TargetSearchType_t searchType = getNumber<TargetSearchType_t>(L, 2, TARGETSEARCH_ANY);
...
int LuaScriptInterface::luaItemTypeIsChest(lua_State * L) 
{
	// itemType:isChest()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushBoolean(L, itemType->isChest());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeIsSplash(lua_State* L) 
{
	// itemType:isSplash()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushBoolean(L, itemType->isSplash());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeIsDisguised(lua_State* L)
{
	// itemType:isDisguised()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushBoolean(L, itemType->disguise);
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaItemTypeIsDestroyable(lua_State * L)
{
	// itemType:isDestroyable()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushBoolean(L, itemType->destroy);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeIsGroundTile(lua_State * L)
{
	...
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushBoolean(L, itemType->group == ITEM_GROUP_GROUND);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeGetDisguiseId(lua_State * L)
{
	// itemType:getDisguiseId()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
...
int LuaScriptInterface::luaItemTypeGetDestroyTarget(lua_State * L)
{
	// itemType:getDestroyTarget()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, itemType->destroyTarget);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemTypeGetNutrition(lua_State* L)
{
	// itemType:getNutrition()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, itemType->nutrition);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaConditionSetSpeedDelta(lua_State* L)
{
	// condition:setSpeedDelta(speedDelta)
	int32_t speedDelta = getNumber<int32_t>(L, 2);
...
int LuaScriptInterface::luaConditionSetTiming(lua_State* L)
{
	// condition:setTiming(count)
	int32_t count = getNumber<int32_t>(L, 2);
	ConditionDamage* condition = dynamic_cast<ConditionDamage*>(getUserdata<Condition>(L, 1));
	if (condition) {
		if (condition->getType() == CONDITION_POISON) {
			condition->setParam(CONDITION_PARAM_COUNT, 3);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 3);
		} else if (condition->getType() == CONDITION_FIRE) {
			condition->setParam(CONDITION_PARAM_COUNT, 8);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 8);

			count /= 10;
		} else if (condition->getType() == CONDITION_ENERGY) {
			condition->setParam(CONDITION_PARAM_COUNT, 10);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 10);

			count /= 20;
		}

		condition->setParam(CONDITION_PARAM_CYCLE, count);
